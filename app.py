...
def generate_prompt_with_openai(prompt_request: str, model: str) -> str:
...
## Korean Notes
OPENAI_API_KEY가 설정되지 않아 기본 프롬프트를 표시했습니다. 상품 카테고리와 선택 이미지에 맞춰 세부 표현을 수동으로 조금 보정한 뒤 Kling에 입력해주세요.
"""
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("OPENAI_API_KEY는 .env 또는 Streamlit secrets에 설정하세요.")
        st.divider()
        st.markdown("### MVP 범위")
        st.write("✅ 이미지 추출")
    ...
    if generate_clicked:
        if not selected_images:
            ...
            with st.spinner("GPT가 Kling용 프롬프트를 생성하는 중입니다..."):
                ...
                try:
                    result = generate_prompt_with_openai(prompt_request, model)
                except Exception as e:
