...
```env
# OpenAI API Key is not stored in .env. Enter it in the Streamlit UI.
```
...
MVP v4.2 does **not** store the OpenAI API Key in `.env`, GitHub, logs, CSV files, or ZIP downloads. Enter the key in the Streamlit sidebar only when generating prompts. The key is used only for the current running session.
